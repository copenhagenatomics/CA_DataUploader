            ...
            static void OrderDecisionsBasedOnIOconf(List<LoopControlDecision> decisions)
            {
                ...
                foreach (var conf in confDecisions)
                {
                    if (!decisionsIndexes.TryGetValue(conf.Name, out var decisionAndIndex)) 
                        throw new FormatException($"decision listed in IO.conf (line {conf.LineNumber + 1}) was not found: {conf.Name}");
                    var decisionVersion = decisionAndIndex.decision.GetType().Assembly.GetName().Version ?? throw new FormatException($"Failed to retrieve assembly version for decision {conf.Name} (line {conf.LineNumber + 1})");
                    if (decisionVersion.Major != conf.Version.Major || decisionVersion.Minor != conf.Version.Minor || decisionVersion.Build != conf.Version.Build )
                        ...
                        throw new FormatException($"decision listed in IO.conf (line {conf.LineNumber + 1}) did not match expected version: {conf.Version} - Actual: {decisionVersion}");
